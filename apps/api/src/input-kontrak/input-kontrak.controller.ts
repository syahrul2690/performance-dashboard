...
class ReviewDto {
  ...
  @IsOptional() @IsIn(["konseptor", "previous"]) returnTo?:
    | "konseptor"
    | "previous";
}
...
class BundleReviewDto {
  @IsIn(["approve", "reject"]) action: "approve" | "reject" = "approve";
  @IsIn(["KP", "UPMK"]) scope: "KP" | "UPMK" = "KP";
...
export class InputKontrakController {
  ...
  @Get("bundle")
  bundle(
    @Query("scope") scope: "KP" | "UPMK" = "KP",
    @Query("year") year?: string,
    @Query("kmType") kmType: string = "draft",
  ) {

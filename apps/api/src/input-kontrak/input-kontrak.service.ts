...
export class InputKontrakService {
  ...
  async save(
    ...
    if (kmType !== "draft" && kmType !== "final")
      throw new BadRequestException('kmType harus "draft" atau "final"');
    // Hanya Kantor Induk yang membuat KM (termasuk KM untuk UPMK). UPMK hanya mengisi realisasi.
    if (user.unit !== "KP")
      throw new ForbiddenException(
        "Kontrak Manajemen hanya dapat dibuat oleh Kantor Induk",
      );
    ...
    if (
      ...
      throw new ForbiddenException(
        "Anda hanya dapat menyusun Kontrak Manajemen pada bidang Anda",
      );
    }
    ...
    let result;
    if (id) {
      const existing = await this.prisma.kontrakManajemen.findUnique({
        where: { id },
      });
      if (!existing) throw new NotFoundException("Kontrak tidak ditemukan");
      if (existing.status === "submitted" || existing.status === "approved") {
        throw new ForbiddenException(
          "Kontrak yang sudah dikirim tidak dapat diubah",
        );
      }
    ...
    await this.prisma.auditLog.create({
      data: {
        actor: user.name,
        userId: user.id,
        action: id ? "kontrak.update" : "kontrak.create",
  ...
  async getReviewerCandidates() {
    const users = await this.prisma.user.findMany({
      where: {
        isActive: true,
        role: { in: [...CHECKER_ROLES, ...APPROVER_ROLES] },
      },
  ...
  async submit(
    ...
    if (user.unit !== "KP")
      throw new ForbiddenException(
        "Kontrak Manajemen hanya dapat dikirim oleh Kantor Induk",
      );
    const kontrak = await this.prisma.kontrakManajemen.findUnique({
      where: { id },
    });
    if (!kontrak) throw new NotFoundException("Kontrak tidak ditemukan");
    if (!Array.isArray(checkerIds) || checkerIds.length === 0)
      throw new BadRequestException("Pilih minimal satu Checker");
    if (!Array.isArray(approverIds) || approverIds.length === 0)
      throw new BadRequestException("Pilih minimal satu Approver");
    // Hanya penyusun (pembuat draft) yang dapat mengirim KM ini.
    if (kontrak.submitterId && kontrak.submitterId !== user.id) {
      throw new ForbiddenException(
        "Hanya penyusun KM ini yang dapat mengirimnya",
      );
    }
    ...
    const checkers = checkerIds
      .map((cid) => picked.find((u) => u.id === cid))
      .filter(Boolean) as ReviewerParticipant[];
    const approvers = approverIds
      .map((aid) => picked.find((u) => u.id === aid))
      .filter(Boolean) as ReviewerParticipant[];
    if (checkers.length !== checkerIds.length)
      throw new BadRequestException(
        "Sebagian Checker tidak ditemukan atau nonaktif",
      );
    if (approvers.length !== approverIds.length)
      throw new BadRequestException(
        "Sebagian Approver tidak ditemukan atau nonaktif",
      );

    const submitter: ReviewerParticipant = {
      id: user.id,
      name: user.name,
      role: user.role,
      unit: user.unit,
    ...
    const invalid = validateReviewerSelection(
      user.id,
      checkers,
      approvers,
      !!srManajer,
  ...
  async delete(user: User, id: string) {
    ...
    if (kontrak.status === "submitted" || kontrak.status === "approved") {
      throw new ForbiddenException(
        "Kontrak yang sudah dikirim tidak dapat dihapus",
      );
    }
  ...
  async getReviewList(user: User) {
    ...
    return submitted
      ...
      .map((r) => {
        const steps = (r.steps as unknown as Step[]) ?? [];
        // View untuk Checker/Approver: sembunyikan langkah 'submitter' (bukan tahap review),
        // dan sediakan index yang SUDAH relatif terhadap array yang disaring ini — sama
        // pattern seperti InputKontrakService.getReviewList().
        const reviewSteps = steps.filter((s) => s.kind !== "submitter");
        const reviewStepIndex = Math.max(0, r.currentStepIndex - 1);
        return {
          ...r,
          reviewSteps,
          reviewStepIndex,
          stepLabel: steps[r.currentStepIndex]?.label ?? "—",
          slaRemainingDays: slaRemainingDays(r),
        };
      });
  }
  ...
  async review(
    ...
    if (k.status !== "submitted")
      throw new ForbiddenException(
        "Kontrak tidak dalam status menunggu review",
      );
    if (!note?.trim())
      throw new BadRequestException(
        "Catatan/komentar wajib diisi saat menyetujui atau menolak",
      );
    ...
    if (!stepMatches(steps[curIdx], user)) {
      throw new ForbiddenException(
        `Usulan ini menunggu langkah "${steps[curIdx]?.label ?? "lain"}", bukan langkah Anda`,
      );
    }
    ...
    if (action === "reject") {
      ...
      const history = [
        ...baseHistory,
        {
          ...
          actor: user.name,
          role: user.role,
          action: toPrev ? "returned_step" : "returned",
          toStepIndex: newIdx,
          label: steps[curIdx]?.label,
          note,
          ts: new Date().toISOString(),
        },
      ];
      ...
      await this.prisma.auditLog.create({
        data: {
          actor: user.name,
          userId: user.id,
          action: toPrev ? "kontrak.return_step" : "kontrak.reject",
          entity: "KontrakManajemen",
          targetId: id,
          note,
        },
    ...
    if (chainDone) {
      ...
      if (yr) {
        ...
        await this.prisma.kMBundle.updateMany({
          where: { year: yr, scope, status: { in: ["rejected", "approved"] } },
          data: { status: "open" },
        });
      }
  ...
  async bulkApprove(user: User, note: string) {
    if (!note?.trim())
      throw new BadRequestException(
        "Catatan/komentar wajib diisi saat menyetujui",
      );
  ...
  async updateKpiItems(user: User, id: string, kpiItems: object[]) {
    ...
    if (km.status !== "submitted")
      throw new ForbiddenException(
        "Hanya KM yang sedang direview yang dapat diedit",
      );
    ...
    const result = await this.prisma.kontrakManajemen.update({
      where: { id },
      data: { kpiItems: kpiItems as object, history },
    });
    await this.prisma.auditLog.create({
      data: {
        actor: user.name,
  ...
  private async resolveYear(year?: string) {
    if (year) return year;
    const active = await this.prisma.period.findFirst({
      where: { isActive: true },
    });
    return active
      ? active.yearMonth.slice(0, 4)
  ...
  async getBundle(
    ...
    const unitFilter =
      scope === "KP" ? { unitCode: "KP" } : { unitCode: { not: "KP" } };
    ...
    const allApproved =
      components.length > 0 && components.every((c) => c.status === "approved");
    ...
    const assignments = masterIds.length
      ? await this.prisma.kpiAssignment.findMany({
          where: { kpiMasterId: { in: masterIds } },
        })
      : [];
    const persenLookup = new Map(
      assignments.map((a) => [
        `${a.kpiMasterId}|${a.unitCode}|${a.bidang}`,
        a.persenAgregasi,
      ]),
    );
    return {
      ...
      components: components.map((c) => ({
        ...
        kpiItems: (Array.isArray(c.kpiItems)
          ...
          ...it,
          persenAgregasi: persenLookup.get(
            `${it["masterKpiId"]}|${c.unitCode}|${c.bidang}`,
          ),
        })),
      })),
    };
  ...
  async reviewBundle(
    ...
    if (user.role !== Role.GM)
      throw new ForbiddenException(
        "Hanya General Manager yang menyetujui bundle KM",
      );
    if (!note?.trim())
      throw new BadRequestException("Catatan/komentar wajib diisi");
    const yr = await this.resolveYear(year);
    const pids = await this.periodIdsForYear(yr);
    const unitFilter =
      scope === "KP" ? { unitCode: "KP" } : { unitCode: { not: "KP" } };
    ...
    if (components.length === 0)
      throw new BadRequestException(
        `Tidak ada KM ${scopeLabel} yang siap dikonsolidasi pada tahun ini`,
      );

    if (action === "approve") {
      if (!components.every((c) => c.status === "ready"))
        throw new ForbiddenException(
          `Masih ada KM ${scopeLabel} yang belum lolos sampai SM Perencanaan & PC`,
        );
      await this.prisma.kontrakManajemen.updateMany({
        where: {
          periodId: { in: pids },
          status: "ready",
          kmType,
      ...
      for (const c of components.filter((x) => x.status === "ready")) {
        ...
        const baseHist = Array.isArray(c.history)
          ? (c.history as object[])
          : [];
        const history = [
          ...baseHist,
          {
            ...
            actor: user.name,
            role: user.role,
            action: "bundle_returned",
            toStepIndex: idx,
            label: cSteps[idx]?.label ?? "Manajer Perencanaan",
            note,
            ts: new Date().toISOString(),
          },
        ];
    ...
    if (action === "approve") {
      const submitterIds = [
        ...new Set(components.map((c) => c.submitterId).filter(Boolean)),
      ] as string[];
      ...
      const manPer = await this.prisma.user.findMany({
        where: { isActive: true, roleVariant: { code: "man_perencanaan" } },
      });
      if (manPer.length) {
        await this.prisma.notification.createMany({
          data: manPer.map((u) => ({
            userId: u.id,
            type: "alert",
            title: "Bundle KM Dikembalikan GM",
